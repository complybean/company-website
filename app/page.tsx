...
const problemPoints = [
  { icon: AlertTriangle, number: '01', title: 'A sequence of high-stakes decisions', text: 'Bringing a medical device to market means making a series of high-stakes regulatory decisions—often without the support of a dedicated regulatory team.' },
  { icon: Route, number: '02', title: 'Every decision shapes the path', text: 'Classification, product code, predicate selection, and supporting evidence all influence the path to an FDA 510(k) submission.' },
  { icon: UsersRound, number: '03', title: 'Small teams carry the uncertainty', text: 'For early-stage companies, this work often falls to founders and multidisciplinary teams, where uncertainty can mean delays, increased costs, and extended time to market.' },
]
    ...
    <main id="top">
      ...
      <section className="border-y bg-card py-20 md:py-28">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="max-w-3xl"><p className="section-kicker mb-5">01 · The problem</p><h2 className="section-title">High-stakes decisions. Limited regulatory support.</h2></div>
          <div className="mt-12 divide-y rounded-4xl border bg-background px-6 shadow-sm md:px-8">
            {problemPoints.map(({ icon: Icon, ...item }) => <article key={item.number} className="grid gap-4 py-7 md:grid-cols-[4rem_.75fr_1.25fr] md:items-start md:gap-8"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-2xl bg-secondary text-primary"><Icon className="size-5" /></span><span className="font-mono text-xs text-primary md:hidden">{item.number}</span></div><h3 className="text-xl font-semibold leading-snug">{item.title}</h3><p className="max-w-2xl leading-7 text-muted-foreground">{item.text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          <ol className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {decisions.map(({ icon: Icon, title, text }, index) => <li key={title} className="soft-card lift-card relative flex min-h-64 flex-col justify-between overflow-hidden rounded-3xl p-6"><span className="absolute right-4 top-2 text-7xl font-semibold text-primary/[.035]">{index + 1}</span><div className="flex items-center justify-between"><span className="flex size-11 items-center justify-center rounded-2xl bg-secondary text-primary"><Icon className="size-5" /></span><span className="font-mono text-xs text-muted-foreground">0{index + 1}</span></div><div><h3 className="text-xl font-semibold">{title}</h3><p className="mt-2 leading-7 text-muted-foreground">{text}</p></div></li>)}
          </ol>
        </div>
      </section>
      ...
      <section className="border-y bg-secondary py-20 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {supportItems.map(({ icon: Icon, ...item }) => <article key={item.title} className="soft-card lift-card rounded-3xl p-6 md:p-8"><span className="mb-10 flex size-12 items-center justify-center rounded-2xl bg-accent"><Icon className="size-5" /></span><h3 className="text-xl font-semibold">{item.title}</h3><p className="mt-3 leading-7 text-muted-foreground">{item.text}</p></article>)}
          </div>
