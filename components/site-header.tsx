  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 md:px-8">
        <Link href="/" className="flex items-center gap-2.5 rounded-lg font-semibold tracking-tight" aria-label="ComplyBean home">
          <Image src="/images/complybean-mark.png" alt="" width={40} height={40} className="size-9 object-contain" priority />
          <span className="text-lg">ComplyBean</span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          <Link href="/#start" className="hidden min-h-11 items-center gap-2 rounded-full bg-foreground px-5 text-sm font-semibold text-background hover:bg-primary sm:inline-flex">
          ...
          <div className="relative md:hidden" ref={menuRef}>
            ...
            {menuOpen && (
              <nav id="mobile-navigation" className="absolute right-0 top-13 flex w-64 flex-col gap-1 rounded-2xl border bg-card p-2 shadow-xl" aria-label="Mobile navigation">
                ...
                <Link href="/#start" onClick={() => setMenuOpen(false)} className="mt-1 flex items-center justify-between rounded-xl bg-primary px-4 py-3 font-semibold text-primary-foreground sm:hidden">
                  Let&apos;s talk <ArrowUpRight className="size-4" />
                </Link>
              </nav>
            )}
